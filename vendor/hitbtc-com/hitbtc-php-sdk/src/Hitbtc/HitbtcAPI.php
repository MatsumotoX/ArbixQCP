...
class HitbtcAPI
{
    ...
    {
        $this->publicKey = config('hitbtc.auth.key');
        $this->secretKey = config('hitbtc.auth.secret');
        $this->host = config('hitbtc.urls.api');
    ...
    public function getOrderbookLimit($symbol,$limit)
    {
        return json_decode($this->getHttpClient()->get('/api/2/public/orderbook/'.$symbol.'?limit='.$limit)->getBody(), true);
     ...
     * @return HttpClient
     */
    public function getHttpClientProtected()
    {
        if (!$this->httpClient) {
    ...
    {
        $response = $this->getHttpClientProtected()->post('/api/1/trading/new_order', array(
            'body' => $order->asArray(),
            'exceptions' => false,
        ));
    ...
    public function cancelOrder(Order $order, $cancelRequestId = null)
    {
        ...
        $response = $this->getHttpClientProtected()->post('/api/1/trading/cancel_order', array(
            'body' => array(
    ...
    public function getActiveOrders($symbols = null)
    {
        ...
        $response = $this->getHttpClientProtected()->get('/api/1/trading/orders/active', $params);
        $document = json_decode($response->getBody(), true);
    ...
    public function getRecentOrders($symbols = null, $sort = 'asc', $statuses = null, $offset = 0, $limit = 1000)
    {
        ...
        $response = $this->getHttpClientProtected()->get('/api/1/trading/orders/recent', array('query' => $query, 'exceptions' => false));
        $document = json_decode($response->getBody(), true);
        if (isset($document['orders'])) {
            $orders = [];
            foreach ($document['orders'] as $orderData) {
    ...
    public function getTrades($symbols = null, $by = 'trade_id', $sort = 'ask', $from = null, $till = null, $offset = 0, $limit = 1000)
    {
        ...
        $response = $this->getHttpClientProtected()->get('/api/1/trading/trades', array('query' => $query, 'exceptions' => false));
        $document = json_decode($response->getBody(), true);
    ...
    public function getTradesByOrder($clientOrderId)
    {
        ...
        $response = $this->getHttpClientProtected()->get('/api/1/trading/trades/by/order', array('query' => $query, 'exceptions' => false));
        $document = json_decode($response->getBody(), true);
        if (isset($document['trades'])) {
            $trades = [];
            foreach ($document['trades'] as $tradeData) {
    ...
    {
        $response = $this->getHttpClientProtected()->get('/api/1/trading/balance', array('exceptions' => false));
        $document = json_decode($response->getBody(), true);
    ...
    {
        $response = $this->getHttpClientProtected()->get('/api/1/payment/balance', array('exceptions' => false));
        $document = json_decode($response->getBody(), true);
        if (isset($document['balance'])) {
            $balances = [];
            foreach ($document['balance'] as $balanceData) {
    ...
    {
        if ($new) {
            $response = $this->getHttpClientProtected()->post('/api/1/payment/address/' . $currency, array('exceptions' => false));
        } else {
            $response = $this->getHttpClientProtected()->get('/api/1/payment/address/' . $currency, array('exceptions' => false));
        }
        $document = json_decode($response->getBody(), true);
    ...
    protected function _transferAmount($currency, $amount, $trading)
    {
        ...
        $response = $this->getHttpClientProtected()->post($url, array(
            'body' => array(
    ...
    {
        $response = $this->getHttpClientProtected()->post('/api/1/payment/payout', array(
            'body' => array(
                'currency_code' => $currency,
                'amount' => $amount,
                'address' => $address,
    ...
    public function getTransactions($sort = 'asc', $offset = 0, $limit = 1000)
    {
        ...
        $response = $this->getHttpClientProtected()->get('/api/1/payment/transactions', array('query' => $query, 'exceptions' => false));
        $document = json_decode($response->getBody(), true);
